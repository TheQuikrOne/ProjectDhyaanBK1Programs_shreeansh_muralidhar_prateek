% summarizeAlphaResults - Aggregate new alpha results + pair-level counts.
...
fprintf('\n=== Pair-level direction counts (Med - Con) ===\n');
for pIdx = 1:numProtocols
    ...
    for m=1:size(metrics,1)
        ...
        if nNonZero > 0
            kk = min(nGreater, nLess);
            p = 2 * binocdf(kk, nNonZero, 0.5); p = min(p, 1);
        else; p = NaN; end
